...
// Regular expression to find SQL injection attack using SQL sleep function
static __always_inline int lex(uint8_t *pkt_data, uint32_t payload_len) {
  {
    ...
    yy1:
        if (i == off) return 0;
        if (yych == 'U') {
            if (i + 10 >= off) return 0;
            if (*(pkt_data + 1) == 's' && *(pkt_data + 2) == 'e' &&
                *(pkt_data + 3) == 'r' && *(pkt_data + 4) == '-' &&
                *(pkt_data + 5) == 'A' && *(pkt_data + 6) == 'g' &&
                *(pkt_data + 7) == 'e' && *(pkt_data + 8) == 'n' &&
                *(pkt_data + 9) == 't' && *(pkt_data + 10) == ':'
            ) goto yy2;
        }
        yych = *++pkt_data;
        i += 1;
    ...
    yy3:
        ...
        i += 1;
        if (i + 5 > off) return 0;
        if (yych == 'l' && *(pkt_data + 1) == 'e' &&
          *(pkt_data + 2) == 'e' && *(pkt_data + 3) == 'p' &&
          *(pkt_data + 4) == '(') return DROP;
        goto yy2;
  }
}
...
static __always_inline int process_packet(struct packet *pkt, __u64 off) {
    ...
    //Tests with pktgen comment these lines.
    //It doesn't generate HTTP packets.
